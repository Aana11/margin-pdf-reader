...
if (
  ...
  throw new Error(
    'Usage: node scripts/benchmark-index-store.mjs [chunks] [dimensions]',
  );
...
function entryAt(index) {
  const vector = Array.from(
    { length: dimensions },
    (_, dimension) => ((index * 31 + dimension * 17) % 997) / 997,
  );
  return {
    id: `p${Math.floor(index / 2) + 1}-c${index % 2}`,
    page: Math.floor(index / 2) + 1,
    text:
      `Synthetic large-book chunk ${index}. ` + '索引性能测试文本。'.repeat(8),
    vector,
  };
}
...
try {
  ...
  await writeFile(
    path.join(legacyDirectory, 'index.json'),
    JSON.stringify({
      version: 1,
      providerId,
  ...
  for (let start = 0; start < entries.length; start += 16)
    appendIndexBatch(build, entries.slice(start, start + 16));
  const sqlite = finishIndexBuild(build);
  const sqliteMs = performance.now() - sqliteStarted;

  const query = Float32Array.from(
    entries[Math.floor(entries.length / 2)].vector,
  );
  const searchStarted = performance.now();
  const matches = searchIndex(sqliteDirectory, providerId, query, 5);
  const searchMs = performance.now() - searchStarted;
  if (matches.length !== 5)
    throw new Error('SQLite search returned an unexpected result count');
  const middlePage = Math.floor(chunks / 4);
  const filteredStarted = performance.now();
  const filteredMatches = searchIndex(sqliteDirectory, providerId, query, 5, {
    pageFrom: middlePage,
    pageTo: middlePage + 19,
  });
  const filteredSearchMs = performance.now() - filteredStarted;
  if (
    filteredMatches.some(
      (match) => match.page < middlePage || match.page > middlePage + 19,
    )
  )
    throw new Error('SQLite filtered search escaped the requested page range');
  ...
  const report = {
    ...
    sqlite: {
      bytes: sqlite.bytes,
      writeMs: Math.round(sqliteMs),
      searchMs: Math.round(searchMs),
      filteredSearchMs: Math.round(filteredSearchMs),
    },
    migration: { bytes: migrated.bytes, elapsedMs: Math.round(migrationMs) },
    sizeReductionPercent: Number(
      ((1 - sqlite.bytes / jsonBytes) * 100).toFixed(1),
    ),
