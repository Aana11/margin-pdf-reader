...
const resumed = queueBooks(interrupted, [
  {
    id: interrupted[0].bookId,
    name: interrupted[0].bookName,
    pageCount: 421,
    lastPage: 1,
...
assert.equal(resumed[0].ocrPageFrom, 20);
assert.equal(resumed[0].ocrPageTo, 28);
assert.equal(resumed[0].forceOcr, true);
assert.equal(resumed[0].glmRefine, true);

assert.equal(shouldInspectWithOcr(''), true);
assert.equal(shouldInspectWithOcr('12'), true);
assert.equal(
  shouldInspectWithOcr(
    '这是一段可以直接从 PDF 中提取的、具有足够长度的正常正文，因此无需再做 OCR。',
  ),
  false,
);
...
console.log(
  JSON.stringify({
    interrupted: interrupted[0].status,
    resumed: resumed[0].progress,
    classifier: 'passed',
  }),
);
