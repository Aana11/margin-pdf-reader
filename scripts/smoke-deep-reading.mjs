...
const api = createServer((request, response) => {
  ...
  request.on('end', () => {
    ...
    if (request.url === '/api/tags') {
      response.end(
        JSON.stringify({
          models: [{ name: 'glm-ocr:latest', model: 'glm-ocr:latest' }],
        }),
      );
      return;
    }
    ...
    if (request.url === '/api/generate' && body.model === 'glm-ocr:latest') {
      ...
      response.end(
        JSON.stringify({
          response: '\\int_0^1 x^2\\,dx = \\frac{1}{3}',
          done: true,
        }),
      );
      return;
...
const baseEndpoint = `http://127.0.0.1:${apiAddress.port}`;
...
try {
  ...
  await retry(() =>
    evaluate(
      `document.querySelector('.page-scroll-indicator')?.textContent || ''`,
    ).then((text) => {
      if (!text.includes('1 / 2')) throw new Error('PDF not open');
      return text;
    }),
  );
  await evaluate(
  ...
  await retry(
    () =>
      evaluate(
        `(async () => ({ indexed: Boolean((await window.marginDesktop.libraryList())[0]?.indexProviderId), status: document.querySelector('.index-strip strong')?.textContent || '', error: document.querySelector('.error-message')?.textContent || '' }))()`,
        true,
      ).then((state) => {
        if (state.error) throw new Error(state.error);
        if (!state.indexed) throw new Error('Index not ready');
        return state;
      }),
    180,
  );
  await evaluate(
    `(() => { const input = document.querySelector('[aria-label="输入问题"]'); const setter = Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value').set; setter.call(input, '请精读向量命中页中的公式并解释'); input.dispatchEvent(new Event('input', { bubbles: true })); })()`,
  );
  await evaluate(`document.querySelector('[aria-label="发送"]')?.click()`);
  const ui = await retry(
    () =>
      evaluate(
        `({ answer: [...document.querySelectorAll('.message.assistant .message-content')].at(-1)?.textContent || '', formulaRendered: Boolean(document.querySelector('.message.assistant .katex')), status: document.querySelector('.assistant-model-status')?.textContent || '', error: document.querySelector('.error-message')?.textContent || '' })`,
      ).then((state) => {
        if (state.error) throw new Error(state.error);
        if (!state.answer.includes('GLM-OCR'))
          throw new Error('Answer not ready');
        return state;
      }),
    180,
  );
  ...
  assert.match(requests.glm[0].images[0], /^[A-Za-z0-9+/=]+$/);
  assert.match(requests.chat[0].messages.at(-1).content, /\\int_0\^1/);
  assert.match(ui.status, /已精读 2 页/);
  assert.equal(ui.formulaRendered, true);
  console.log(
    JSON.stringify({
      embeddingInputs: requests.embedding,
      glmRequests: requests.glm.length,
