...
const api = createServer((request, response) => {
  ...
  request.on('end', () => {
    ...
    if (request.url === '/api/tags') {
      response.end(JSON.stringify({ models: [{ name: 'glm-ocr:latest', model: 'glm-ocr:latest' }] }));
      return;
    }
...
try {
  ...
  await retry(() => evaluate(`document.querySelector('.page-scroll-indicator')?.textContent || ''`).then((text) => { if (!text.includes('1 / 2')) throw new Error('PDF not open'); return text; }));
