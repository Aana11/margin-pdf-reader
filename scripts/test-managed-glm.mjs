...
const dataRoot = process.env.MARGIN_MANAGED_GLM_ROOT || path.join(process.env.APPDATA, 'Margin');
const profileRoot = path.resolve('tmp', 'managed-glm-profile');
const cdpPort = Number(process.env.MARGIN_GLM_CDP_PORT || 9341);
const child = spawn(executable, ['--disable-gpu', `--remote-debugging-port=${cdpPort}`, `--user-data-dir=${profileRoot}`], {
  stdio: 'ignore',
  env: { ...process.env, MARGIN_DATA_ROOT: dataRoot, MARGIN_LIBRARY_ROOT: path.resolve('tmp', 'managed-glm-library') },
});
...
try {
  ...
  const prepared = await evaluate(`window.marginDesktop.glmOcrPrepare(${config})`, true);
  ...
  const unloaded = await evaluate(`window.marginDesktop.glmOcrUnload(${config})`, true);
  assert.equal(unloaded?.modelLoaded, false);
  console.log(JSON.stringify({ stage: 'complete', recognized: result.text.slice(0, 500), unloaded: !unloaded.modelLoaded }));
