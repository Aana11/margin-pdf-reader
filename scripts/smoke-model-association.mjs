...
const qwenRoot = path.join(dataRoot, 'models', 'Qwen', 'Qwen3-Embedding-4B-GGUF');
await Promise.all([mkdir(runtimeRoot, { recursive: true }), mkdir(glmRoot, { recursive: true }), mkdir(qwenRoot, { recursive: true })]);
await writeFile(path.join(runtimeRoot, 'llama-server.exe'), 'fixture');
await writeFile(path.join(runtimeRoot, '.margin-runtime-version'), 'cpu:b10516');
await writeFile(path.join(qwenRoot, 'Qwen3-Embedding-4B-Q4_K_M.gguf'), 'fixture');
...
const child = spawn(executable, ['--disable-gpu', `--remote-debugging-port=${cdpPort}`, `--user-data-dir=${path.join(root, 'profile')}`], {
  stdio: 'ignore',
  env: { ...process.env, MARGIN_DATA_ROOT: dataRoot, MARGIN_LIBRARY_ROOT: path.join(root, 'library'), MARGIN_RUNTIME_BACKEND: 'cpu' },
});
...
try {
  ...
  const associated = await retry(() => evaluate(`(async () => {
    ...
    if (state.settings.glmOcrMode !== 'auto') throw new Error('Managed GLM-OCR has not been auto-associated yet');
    return state;
  }));
  ...
  assert.equal(associated.app.version, '0.2.8');
  assert.equal(associated.app.dataRoot, dataRoot);

  await evaluate(`(() => { const settings = JSON.parse(localStorage.getItem('margin-ai-settings')); settings.glmOcrMode = 'off'; localStorage.setItem('margin-ai-settings', JSON.stringify(settings)); window.location.reload(); })()`);
  const preserved = await retry(() => evaluate(`document.readyState === 'complete' ? JSON.parse(localStorage.getItem('margin-ai-settings') || '{}').glmOcrMode : ''`).then((mode) => {
    if (!mode) throw new Error('Reload pending');
    return mode;
  }));
  assert.equal(preserved, 'off');
  console.log(JSON.stringify({ schema: associated.schema, embedding: associated.embedding.installed, glm: associated.glm.modelInstalled, autoMode: associated.settings.glmOcrMode, explicitOff: preserved }));
} finally {
