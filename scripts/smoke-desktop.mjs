...
const pdfPath = path.resolve(
  process.argv[2] || 'tmp/pdfs/margin-reader-smoke.pdf',
);
...
const child = attachToRunningApp
  ...
  : spawn(
      ...
      [
        ...
        `--user-data-dir=${path.join(libraryRoot, 'profile')}`,
      ],
      {
        stdio: 'ignore',
        env: {
...
async function openDialog(triggerLabel, expectedText) {
  return retry(
    async () => {
      ...
      if (!state.open || !state.text.includes(expectedText))
        throw new Error(`${expectedText} dialog is not open yet`);
      return state;
    },
    120,
    500,
...
try {
  ...
  const pageOne = await retry(
    async () => {
      ...
      if (
        ...
        state.hasReadingBanner ||
        !state.hasPageIndicator
      )
        throw new Error(
          `Page 1 or scroll region has not rendered yet: ${JSON.stringify(state)}`,
        );
      return state;
    },
    120,
  ...
  await evaluate(
    `(() => { const reader = document.querySelector('.canvas-wrap'); if (!reader) return; reader.style.scrollBehavior = 'auto'; reader.scrollTop = reader.scrollHeight; reader.dispatchEvent(new Event('scroll')); })()`,
  );
  const pageTwo = await retry(
    async () => {
      const state = await evaluate(`({
      page: document.querySelector('.page-scroll-indicator')?.textContent || '',
      ...
      if (
        !state.page.includes('2 / 2') ||
        !state.synced.includes('2') ||
        state.canvasWidth <= 1
      )
        throw new Error(
          `Page 2 has not rendered yet: ${JSON.stringify(state)}`,
        );
      return state;
    },
    120,
  ...
  const ocrImageBase64 = (
    await readFile(path.resolve('docs', 'images', 'reader-overview.jpg'))
  ).toString('base64');
  ...
  await retry(
    async () => {
      const entries = await evaluate(
        `window.marginDesktop.libraryList()`,
        true,
      );
      if (entries?.[0]?.lastPage !== 2)
        throw new Error('Reading progress has not persisted yet');
      return entries[0];
    },
    30,
    500,
  ...
  await delay(100);
  await evaluate(
    `[...document.querySelectorAll('button')].find((button) => button.textContent?.includes('保存配置'))?.click()`,
  );
  const savedPrompt = await evaluate(
    `JSON.parse(localStorage.getItem('margin-ai-settings') || '{}').systemPrompt || ''`,
  );
  ...
  if (testEmbedding) {
    ...
    if (
      vectors?.length !== 8 ||
      vectors.some((vector) => vector.length !== 2560)
    )
      throw new Error(
        `Unexpected batched embeddings: ${vectors?.length} x ${embeddingDimensions}`,
      );
    await evaluate(
      `[...document.querySelectorAll('button')].find((button) => button.textContent?.includes('建立索引'))?.click()`,
    );
    indexStatus = await retry(
      async () => {
        const state = await evaluate(`({
        ...
        if (state.error) throw new Error(state.error);
        if (!state.label.includes('全文索引已就绪'))
          throw new Error(`Index is not ready: ${state.label}`);
        return state.label;
      },
      360,
      500,
    ...
    if (!modelLoaded?.loaded)
      throw new Error('Local model process is not reported as loaded');
  }
  const historySeed = await evaluate(
    `(async () => {
    ...
    localStorage.removeItem('margin-workspace-sqlite-v1');
  ...
  const persistedHistory = await retry(async () => {
    const text = await evaluate(
      `document.querySelector('.history-item')?.textContent || ''`,
    );
    if (!text.includes('冒烟测试提问') || !text.includes('第 2 页'))
      throw new Error('Question history did not persist');
    return text;
  });
  await closeDialog();
  const workspaceSeed = await evaluate(
    `(async () => {
    const book = (await window.marginDesktop.libraryList())[0];
    const highlight = await window.marginDesktop.workspaceNoteSave(book.id, book.name, { kind: 'highlight', page: 2, title: '冒烟高亮', content: '', excerpt: '第二页重点区域', color: '#f4cf65', region: { x: 0.1, y: 0.1, width: 0.3, height: 0.1 } });
    const summary = await window.marginDesktop.workspaceNoteSave(book.id, book.name, { kind: 'summary', page: 2, title: '冒烟摘要卡片', content: '这是用于验证 SQLite 阅读资料的摘要。', excerpt: '', color: '#8cc8ef' });
    const notes = await window.marginDesktop.workspaceNotesList({ bookId: book.id, query: '冒烟', limit: 50, offset: 0 });
    const history = await window.marginDesktop.workspaceHistorySearch('冒烟测试', 50, 0);
    return { bookId: book.id, highlight: highlight.kind, summary: summary.kind, notes: notes.total, history: history.total };
  })()`,
    true,
  );
  if (
    workspaceSeed.highlight !== 'highlight' ||
    workspaceSeed.summary !== 'summary' ||
    workspaceSeed.notes !== 2 ||
    workspaceSeed.history !== 1
  )
    throw new Error(
      `Workspace SQLite bridge failed: ${JSON.stringify(workspaceSeed)}`,
    );
  await openDialog('阅读资料', '高亮、批注、AI 摘要卡片和概念词典');
  const workspaceUi = await retry(async () => {
    const state = await evaluate(
      `({ text: document.querySelector('.workspace-dialog')?.textContent || '', filters: document.querySelectorAll('.workspace-filters button').length })`,
    );
    if (
      !state.text.includes('冒烟高亮') ||
      !state.text.includes('冒烟摘要卡片') ||
      state.filters !== 5
    )
      throw new Error(
        `Reading workspace is not ready: ${JSON.stringify(state)}`,
      );
    return state;
  });
  await closeDialog();
  ...
  const reopened = await retry(
    async () => {
      ...
      if (
        !state.page.includes('2 / 2') ||
        state.canvasWidth <= 1 ||
        (testEmbedding && !state.index.includes('全文索引已就绪'))
      )
        throw new Error(
          `Persisted book or index has not reopened yet: ${JSON.stringify(state)}`,
        );
      return state;
    },
    240,
  ...
  if (testEmbedding) {
    ...
    if (modelReleased?.loaded)
      throw new Error('Local model process did not release memory');
  }
  await openDialog('本地书架', '集中管理保存在本机的 PDF');
  await evaluate(
    `(() => { window.confirm = () => true; document.querySelector('.book-remove')?.click(); })()`,
  );
  const removed = await retry(
    async () => {
      const state = await evaluate(
        `(async () => ({ books: document.querySelectorAll('.book-item').length, hasPdf: Boolean(document.querySelector('.pdf-pages')), history: (await window.marginDesktop.workspaceHistorySearch('冒烟测试', 50, 0)).total, notes: (await window.marginDesktop.workspaceNotesList({ bookId: ${JSON.stringify(historySeed.bookId)}, limit: 50 })).total }))()`,
        true,
      );
      if (
        state.books !== 0 ||
        ...
        throw new Error(
          'Book removal or workspace cleanup has not completed yet',
        );
      return true;
    },
    60,
