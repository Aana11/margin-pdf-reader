...
![Version](https://img.shields.io/badge/version-0.3.1-258A7B)
...
支持兼容 OpenAI 协议的 `POST /chat/completions`；如果端点提供 `GET /models`，Margin 可以读取模型列表。系统提示词会作为每次请求的首条 `system` 消息。
...
- OCR 主要用于检索和 AI 上下文，还不会为原 PDF 叠加可选文字层。
...
- **0.3.2：** OCR 版面分析、手动页范围、公式批量精修与可选文字层导出。
- **0.3.3：** 保存检索问题、证据看板、跨书对照阅读和带引用的大纲生成。
- **0.3.x：** 继续完善大知识包检索、模型资源调度、备份恢复和阅读体验。
