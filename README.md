...
| 模块化侧边栏与书架 | 固定 64px 左侧栏为后续功能预留模块位置；点击“书架”后通过弹窗导入、打开或移除书籍。PDF、阅读进度和索引均保存在本机。 |
| 当前页问答 | 阅读助手自动附带当前页文本，不需要截图或手动复制。 |
| 全文 RAG | 按页提取文本、切分片段、生成向量并进行余弦相似度检索，结果保留原始页码。 |
| GLM-OCR 联动精读 | 向量检索先定位相关页；本机 Ollama 可由应用检测、启动、下载/载入模型和释放显存，vLLM/SGLang 仍可作为外部端点。 |
...
1. **左侧功能栏：** “书架”以弹窗管理本地书籍，中间预留更多功能模块，最底部固定为“设置”。
...
2. 建立索引时根据逻辑处理器使用 2–8 路文本提取；纯图片页通过 1–4 路本地 OCR 流水线识别，再按页切分为重叠片段。
...
索引写入使用临时 SQLite 检查点，进度只会向前增长；中途点击“暂停”、关闭应用或发生错误会保留已提取/OCR 的页面与已生成向量，下次从断点继续。完整索引仍在成功后原子替换，旧版 `index.json` 会在首次打开时自动迁移。大型书籍结果见 [索引性能基准](docs/index-benchmark.md)。
...
在“模型设置 → 公式、代码与表格精读”中启用 GLM-OCR。本机模式默认使用 Ollama：

```text
运行方式：本机 Ollama（可自动启停）
端点：http://127.0.0.1:11434
模型：glm-ocr:latest
```

如果状态显示“未安装 Ollama”，点击“安装 Ollama”完成官方运行时安装；回到 Margin 后点击“准备模型”，应用会启动本地服务、通过 Ollama 拉取 `glm-ocr:latest` 并预载模型。之后启用“自动开启”，提问命中复杂页面时会自动启动服务和加载模型。Ollama 请求走 GLM-OCR 官方推荐的原生 `/api/generate` 图像接口。

也可选择外部 OpenAI-compatible 模式，填写官方支持的 vLLM/SGLang 服务，例如 `http://127.0.0.1:8080/v1` 与模型名 `glm-ocr`。Margin 不把 GLM-OCR 权重打进安装包；本机 Ollama 模型按需下载，远程端点则会收到最多 2 个候选页图片。精读失败会显示“未安装运行时 / 服务未启动 / 模型未下载 / 端点超时”等具体原因，并自动回退普通检索。
...
- **GLM-OCR 模型按需下载：** 模型默认关闭且不随安装包分发；本机自动管理仍要求先安装 Ollama，外部模式要求已有 vLLM/SGLang 服务。自动模式一次最多精读 2 页，不等同于整书布局解析。
