...
建立索引前会分别检查 GGUF 与 llama.cpp。若运行时目录缺失但已校验的压缩包仍在，应用会自动重新解压；无法恢复时，错误会给出缺失组件和完整目标路径。

本地模型只在生成向量时载入内存，空闲 2 分钟后自动退出；也可在模型设置中立即点击“释放内存”。内置 llama.cpp 上下文与批量大小按 PDF 检索场景收紧，向量索引在渲染进程内使用 Float32 存储。
