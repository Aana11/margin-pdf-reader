...
| 模块化侧边栏与书架 | 固定 64px 左侧栏承载书架、索引任务和阅读资料；点击“书架”后通过弹窗导入、打开或移除书籍。PDF、阅读进度和索引均保存在本机。            |
...
| 阅读资料           | 框选任意页面区域即可保存高亮、批注或概念词条；AI 回答可一键保存为摘要卡片，点击资料能回到原页和原选区。                              |
| 提问历史           | 问答按书籍保存在 `workspace.sqlite`，可跨书搜索、恢复整段对话并跳回当时提问的页面。                                                  |
| Markdown 导出      | 阅读资料与问答历史可通过系统保存窗口导出为 Markdown，公式、代码和表格结构会保留。                                                    |
...
1. **左侧功能栏：** “书架”管理本地书籍，“任务”显示后台索引队列，“资料”集中管理高亮、批注、AI 摘要和概念词典，最底部固定为“设置”。
...
同一个框选工具也承担阅读标注：选择区域后无需启动模型，直接点“高亮”“批注”或“概念”即可保存。完成的 AI 回答下方有“存为摘要卡片”，所有资料都能在左侧“资料”里按类型或关键词检索。
...
| 提问历史、高亮、批注、摘要与概念词典 | `%APPDATA%\Margin\workspace.sqlite`          | 不会自动上传；历史只在后续提问时作为最近对话上下文发送。                              |
| 系统提示词                           | Electron 浏览器存储                          | 不会自动上传；每次提问时发送给所选对话端点。                                          |
| API Key                              | Electron 浏览器存储                          | 仅随请求发送给对应端点；不会进入 Git。                                                |

> API Key 目前依赖 Electron 浏览器存储，并非独立加密保险库。请只在你信任的 Windows 用户账户中使用，并谨慎选择第三方模型服务。
...
```text
Margin/
├─ workspace.sqlite         # 问答、高亮、批注、摘要卡片与概念词典
...
| `npm run test:workspace`                       | 验证阅读资料 SQLite、历史搜索、分页、Markdown 生成与按书清理。                               |
