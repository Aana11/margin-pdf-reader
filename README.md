...
![Version](https://img.shields.io/badge/version-0.3.4-258A7B)
...
4. 想检索整本书时，在 **书架** 中为书籍点击索引按钮；后台任务入口也集中在书架顶部。首次使用内置向量模型会提示下载模型。
...
8. 需要整理长期课题时，点击左侧 **研究**，把问题和带来源的回答加入证据板，再生成跨书对照或引用大纲。
9. 需要复习时，点击左侧 **学习**，从当前页或最近一条 AI 回答生成概念、公式和问答卡。
...
0.3.3 新增 **研究工作台**。提问可以保存为待研究问题，AI 回答及其书名、页码来源可以加入证据板。勾选多条材料后，可以让助手进行跨书对照，或生成每个要点均保留原始页码、缺证处明确标记的分层大纲。研究问题、证据和大纲保存在本机 SQLite，也会进入 Markdown 导出。

![Margin 研究工作台](docs/images/research-workbench.jpg)

### 把阅读内容变成可复习的知识

0.3.4 新增 **学习卡片**。Margin 可以从当前页或最近一条 AI 回答中生成概念卡、公式卡和问答卡，并保留来源书籍与页码。复习时先显示题面，翻开答案后选择“忘记、困难、记得或熟练”；程序会在本机记录复习间隔和错误次数，忘记的卡片会更快再次出现。

![Margin 学习卡片](docs/images/study-cards.jpg)

AI 回答里的文字现在也可以直接引用追问：用鼠标选中回答的一部分，输入框上方会出现引用预览。后续问题会同时携带该选段和原始检索上下文，并要求模型重新核对选段，而不是默认此前回答一定正确。
...
一本或多本书可以加入后台队列。切换阅读书籍不会打断任务；暂停、退出或失败后可以从 SQLite 检查点继续，而不是从第一页重来。后台任务按钮位于 **书架** 顶部，任务中心会分别显示文本提取、OCR、向量生成和写入耗时。
...
| 研究工作台       | 保存问题与证据，进行跨书对照并生成引用大纲。       |
| 回答引用追问     | 选中 AI 回答中的局部内容，针对该段继续核对和提问。 |
| 学习卡片         | 生成概念、公式和问答卡，记录错题并安排间隔复习。   |

AI 回答支持 Markdown、代码块、表格和 KaTeX 公式。可识别 `$...$`、`$$...$$`、`\(...\)` 与 `\[...\]`。兼容端点返回的 `reasoning_content`、`reasoning` 或 `thinking` 会显示为可折叠的“思考过程”，正式答案完成后自动收起。
...
- 图像 Token、批量和 KV 缓存有明确上限；显存不足时会自动改用 CPU 视觉投影重试，不再把模型进程崩溃误报成普通网络断开。
- 模型文件与共享运行时分别检测；运行时需要修复时不会再把完整的 GLM-OCR 权重误报为未安装。
- 自动精读一次最多处理 2 条来源。
...
| 问答、来源、研究资料和知识包  | `%APPDATA%\Margin\workspace.sqlite`                  | 不会自动上传。                                     |
| 当前页和检索片段              | 内存                                                 | 只在主动提问时发送给对话端点。                     |
| GLM-OCR 页面或裁剪            | 当前会话内存                                         | 本地托管时不离开设备；远程模式会发送给所选端点。   |
| API Key、模型地址和系统提示词 | `%APPDATA%\Margin\preferences.json` 与 Electron 存储 | Key 只随请求发往对应端点。                         |
...
可以。自动精读失败会显示具体原因并回退到普通文本检索，不会让整次查询直接失效。0.3.3 会限制图像 Token，并在检测到 Vulkan 显存不足时自动切换保护模式重试。仍然失败时可以在设置中检查模型是否下载完整，或打开日志查看详细信息。

### 已经下载模型，为什么软件仍显示未找到？

0.3.2 起安装版和便携版都固定使用 `%APPDATA%\Margin`，不会再因包名差异创建另一套空配置。0.3.3 又把模型运行方式、端点和实际关联目录同步写入原生配置文件。打开 **设置 → 重新检测** 会重新扫描 Qwen、GLM-OCR 和共享 llama.cpp 运行时，自动关联找到的本地模型并持久化设置。

### 如何调整阅读区和助手宽度？

拖动阅读区与 AI 助手之间的竖向分隔条即可自由调整左右比例；双击恢复默认宽度。比例会在下次启动时恢复。应用也会自动重新打开上次阅读的文档并回到保存页码。
...
- **0.3.6：** 书库备份迁移、缺失 PDF 重新关联和数据完整性检查。
- **0.3.x：** 继续完善阅读体验、自动化和稳定性。
